...
void CDBServConn::HandlePdu(CImPdu* pPdu)
{
	switch (pPdu->GetCommandId()) {
        ...
        case CID_BUDDY_LIST_SEARCH_USER_RESPONSE:
			...
			break;
        case CID_BUDDY_LIST_ADD_FRIEND_RESPONSE:
        	_HandleAddFriendResponse(pPdu);
        	break;
        case CID_BUDDY_LIST_ADD_FRIEND_UNREAD_CNT_RESPONSE:
            _HandleAddFriendUnreadCntResponse(pPdu);
            break;
        case CID_BUDDY_LIST_GET_ADD_FRIEND_DATA_RESPONSE:
			_HandleGetAddFriendDataResponse(pPdu);
			break;
        ...
        case CID_BLOG_GET_COMMENT_RESPONSE:
            ...
            break;
        case CID_BUDDY_LIST_AGREE_ADD_FRIEND_RESPONSE:
            _HandleAgreeAddFriendResponse(pPdu);
            break;
...
void CDBServConn::_HandleMsgBlogAck(CImPdu *pPdu)
{
    ...
    uint32_t user_id = blog.user_id();
    CDbAttachData attach_data((uchar_t*)blog.attach_data().c_str(), blog.attach_data().length());
    uint32_t handle = attach_data.GetHandle();

    log("_HandleMsgBlogAck, userId=%u", user_id);

    CMsgConn* pMsgConn = CImUserManager::GetInstance()->GetMsgConnByHandle(user_id, handle);

    if (pMsgConn && pMsgConn->IsOpen())
    {
    	log("send Msg Blog Ack, userId=%u", user_id);
...
void CDBServConn::_HandleAddFriendUnreadCntResponse(CImPdu *pPdu)
{
	IM::Buddy::IMAddFriendUnreadCntRsp msg;
	CHECK_PB_PARSE_MSG(msg.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength()));
...
void CDBServConn::_HandleGetAddFriendDataResponse(CImPdu *pPdu)
{
	IM::Buddy::IMGetAddFriendDataRsp msg;
	CHECK_PB_PARSE_MSG(msg.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength()));
...
void CDBServConn::_HandleAddFriendResponse(CImPdu *pPdu)
{
    IM::Buddy::IMAddFriendRsp msg;
    CHECK_PB_PARSE_MSG(msg.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength()));

    uint32_t user_id = msg.user_id();
    uint32_t friend_id = msg.friend_id();
    CDbAttachData attach_data((uchar_t*)msg.attach_data().c_str(), msg.attach_data().length());
    uint32_t handle = attach_data.GetHandle();
    ...
    IM::Buddy::IMAddFriendData msg2;
    msg2.set_user_id(user_id);
    msg2.set_friend_id(friend_id);
    msg2.set_type(IM::BaseDefine::ADD_FRIEND_REQUEST);
    msg2.set_add_friend_data(msg.add_friend_data());
	CImPdu pdu2;
	pdu2.SetPBMsg(&msg2);
	pdu2.SetServiceId(IM::BaseDefine::SID_BUDDY_LIST);
	pdu2.SetCommandId(IM::BaseDefine::CID_BUDDY_LIST_ADD_FRIEND_DATA);
    ...
    CImUser* pFriendImUser = CImUserManager::GetInstance()->GetImUserById(friend_id);
...
void CDBServConn::_HandleAgreeAddFriendResponse(CImPdu *pPdu)
{
    IM::Buddy::IMAgreeAddFriendRsp msg;
    CHECK_PB_PARSE_MSG(msg.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength()));
    ...
    log("_HandleAgreeAddFriendResponse, userId=%u", user_id);

    CMsgConn* pMsgConn = CImUserManager::GetInstance()->GetMsgConnByHandle(user_id, handle);

    if (pMsgConn && pMsgConn->IsOpen())
    {
    	log("send Agree Add Friend Response, userId=%u", user_id);
        msg.clear_attach_data();
        pPdu->SetPBMsg(&msg);
        pMsgConn->SendPdu(pPdu);
    }
	...
	msg2.set_user_id(user_id);
	msg2.set_friend_id(friend_id);
	msg2.set_type(msg.agree());
	msg2.set_add_friend_data(msg.agree_friend_data());
	CImPdu pdu2;
	pdu2.SetPBMsg(&msg2);
	pdu2.SetServiceId(IM::BaseDefine::SID_BUDDY_LIST);
	pdu2.SetCommandId(IM::BaseDefine::CID_BUDDY_LIST_ADD_FRIEND_DATA);
...
void CDBServConn::_HandleAddBlogCommentResponse(CImPdu *pPdu)
{
    ...
    log("HandleAddBlogCommentResponse, userId=%u", user_id);
...
void CDBServConn::_HandleGetBlogCommentResponse(CImPdu *pPdu)
{
    ...
    log("HandleGetBlogCommentResponse, userId=%u", user_id);
