...
void CMsgConn::HandlePdu(CImPdu* pPdu)
{
	...
	switch (pPdu->GetCommandId()) {
        ...
        case CID_BUDDY_LIST_SEARCH_USER_REQUEST:
            ...
            break;
        case CID_BUDDY_LIST_GET_ADD_FRIEND_DATA_REQUEST:
        	_HandleClientGetAddFriendDataRequest(pPdu);
        	break;
        case CID_BUDDY_LIST_ADD_FRIEND_UNREAD_CNT_REQUEST:
        	_HandleClientAddFriendUnreadCntRequest(pPdu);
            break;
        case CID_BUDDY_LIST_ADD_FRIEND_READ_DATA_ACK:
        	_HandleClientAddFriendReadDataAck(pPdu);
            break;
        case CID_BUDDY_LIST_ADD_FRIEND_REQUEST:
			_HandleClientAddFriendRequest(pPdu );
			break;
        case CID_BUDDY_LIST_AGREE_ADD_FRIEND_REQUEST:
            _HandleClientAgreeAddFriendRequest(pPdu);
            break;
...
void CMsgConn::_HandleClientSearchUserRequest(CImPdu* pPdu)
{
    ...
    log("user_id=%u search %s", GetUserId(), search_user_name.c_str());
...
void CMsgConn::_HandleClientAddFriendRequest(CImPdu* pPdu)
{
    IM::Buddy::IMAddFriendReq msg;
    CHECK_PB_PARSE_MSG(msg.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength()));
    uint32_t friend_id = msg.friend_id();
    log("user_id=%u add friend %u", GetUserId(), friend_id);

    if(friend_id == GetUserId()){
		...
		msgResp.set_friend_id(friend_id);
		msgResp.set_add_friend_data("");
		msgResp.set_attach_data(msg.attach_data());
		CImPdu pdu;
		pdu.SetPBMsg(&msgResp);
		pdu.SetSeqNum(pPdu->GetSeqNum());
		pdu.SetServiceId(IM::BaseDefine::SID_BUDDY_LIST);
		pdu.SetCommandId(IM::BaseDefine::CID_BUDDY_LIST_ADD_FRIEND_RESPONSE);
...
void CMsgConn::_HandleClientAddFriendReadDataAck(CImPdu* pPdu)
{
    IM::Buddy::IMAddFriendReadDataAck msg;
    CHECK_PB_PARSE_MSG(msg.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength()));
...
void CMsgConn::_HandleClientGetAddFriendDataRequest(CImPdu* pPdu)
{
    IM::Buddy::IMGetAddFriendDataReq msg;
    CHECK_PB_PARSE_MSG(msg.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength()));
    uint32_t cnt = msg.msg_cnt();
    log("user_id=%u cnt %u", GetUserId(), cnt);
...
void CMsgConn::_HandleClientAddFriendUnreadCntRequest(CImPdu* pPdu)
{
    IM::Buddy::IMAddFriendUnreadCntReq msg;
    CHECK_PB_PARSE_MSG(msg.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength()));
    log("user_id=%u", GetUserId());
...
void CMsgConn::_HandleClientDelFriendRequest(CImPdu* pPdu)
{
    ...
    CHECK_PB_PARSE_MSG(msg.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength()));
    uint32_t friend_id = msg.friend_id();
    log("user_id=%u del friend %u", GetUserId(), friend_id);
...
void CMsgConn::_HandleClientAgreeAddFriendRequest(CImPdu* pPdu)
{
    IM::Buddy::IMAgreeAddFriendReq msg;
    CHECK_PB_PARSE_MSG(msg.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength()));
    uint32_t friend_id = msg.friend_id();
    log("user_id=%u agree add friend %u", GetUserId(), friend_id);
...
void CMsgConn::_HandleClientFollowUserRequest(CImPdu* pPdu)
{
    ...
    CHECK_PB_PARSE_MSG(msg.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength()));
    uint32_t friend_id = msg.friend_id();
    log("user_id=%u follow user %u", GetUserId(), friend_id);
