...
public:
    ...
    uint32_t addRelation(uint32_t nSmallId, uint32_t nBigId, uint32_t tag);

    bool delFriend(uint32_t nUserId, uint32_t nFriendId);
    bool delFollowUser(uint32_t nUserId, uint32_t nFriendId);
private:
	CRelationModel();
