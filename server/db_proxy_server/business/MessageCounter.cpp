...
namespace DB_PROXY {
    ...
    void clearUnreadMsgCounter(CImPdu* pPdu, uint32_t conn_uuid)
    {
        ...
        if(msg.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength()))
        {
            ...
            //有错误 hdel failed 61
            //CSystemMsgModel::getInstance()->clearSysMsgCounter(nUserId, nFromId);
        }
        else
        {
            log("parse pb failed");
    ...
    void getUnreadAddFriendMsgCounter(CImPdu* pPdu, uint32_t conn_uuid)
	{
		IM::Buddy::IMAddFriendUnreadCntReq msg;
		IM::Buddy::IMAddFriendUnreadCntRsp msgResp;
		if(msg.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength()))
		{
			CImPdu* pPduResp = new CImPdu;
			...
			CSystemMsgModel::getInstance()->getUnreadSysMsgCount(nUserId, nTotalCnt, ADD_FRIEND);

			msgResp.set_user_id(nUserId);
			msgResp.set_unread_cnt(nTotalCnt);
			msgResp.set_attach_data(msg.attach_data());
			pPduResp->SetPBMsg(&msgResp);
			pPduResp->SetSeqNum(pPdu->GetSeqNum());
			pPduResp->SetServiceId(IM::BaseDefine::SID_BUDDY_LIST);
			pPduResp->SetCommandId(IM::BaseDefine::CID_BUDDY_LIST_ADD_FRIEND_UNREAD_CNT_RESPONSE);
			CProxyConn::AddResponsePdu(conn_uuid, pPduResp);
		}
		else
		{
	...
	void clearAddFriendMsgCounter(CImPdu* pPdu, uint32_t conn_uuid)
	{
		IM::Buddy::IMAddFriendReadDataAck msg;
		if(msg.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength()))
		{
			uint32_t nUserId = msg.user_id();
			CSystemMsgModel::getInstance()->clearSysMsgCounter(nUserId, ADD_FRIEND);
		}
		else
		{
			log("parse pb failed");
