...
int CAudioModel::saveAudioInfo(uint32_t nFromId, uint32_t nToId, uint32_t nCreateTime, const char* pAudioData, uint32_t nAudioLen)
{
    ...
    string strPath = "im/audio/"+int2string(t_tm->tm_year+1900)+"/"+
    		int2string(t_tm->tm_mon+1)+"/"+int2string(nFromId)+"_"+int2string(time(NULL));

	if (pAliyunOss->put_object_from_buffer(strPath, pRealData, nRealLen))
    {
