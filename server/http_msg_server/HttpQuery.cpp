...
void CHttpQuery::DispatchQuery(std::string& url, std::string& post_data, CHttpConn* pHttpConn)
{
	...
	char Arg[256];
	const char* pos = memfind(post_data.c_str(), post_data.length(), "arg=", strlen("arg="));
	if (pos == NULL)
	{
		log("DispatchQuery, not find landu im arg=");
		pHttpConn->Close();
		return;
	}

	/*const char* pos2 = pos + strlen("arg=") + 2;
	int nTmpLen = pos2 - post_data.c_str();
	//log("DispatchQuery, %s",pos2);
	const char * pPosQuotes = memfind(pos2, nTmpLen, "--", strlen("--"));
	if(pPosQuotes == NULL)
	{
		//log("DispatchQuery, landu_arg= error");
		pHttpConn->Close();
		return;
	}
	int nArgLen = pPosQuotes - pos2;

	if(nArgLen <= 255)
	{
		memcpy(Arg,  pos2, nArgLen);
		Arg[nArgLen] = 0;
		//log("DispatchQuery, nArgLen %d, Arg %s quotes %s", nArgLen, Arg, pPosQuotes);
	}else{
		log("DispatchQuery, landu_arg len error");
		pHttpConn->Close();
		return;
	}*/

	string tmp = pos + 4; //跳过 "arg="
	string arg = UrlDecode(tmp);

	Json::Reader reader;
	Json::Value value;
	if ( !reader.parse(arg, value) ) {
		log("json parse failed, arg=%s ", arg.c_str());
		pHttpConn->Close();
		return;
	}

    string strAppKey = "landu";
    //{"user_name":"Tom2","password":"52c69e3a57331081823331c4e69d3f2e"} 999999
    ...
    {
        _QueryRegister(strAppKey, value, pHttpConn);
    }
    else if (strcmp(url.c_str(), "/query/valid_code") == 0)
    {
        //_QueryValidCode(strAppKey, value, pHttpConn);
    }
    else if (strcmp(url.c_str(), "/query/assert_valid_code") == 0)
	{
		//_AssertValidCode(strAppKey, value, pHttpConn);
	}
    else
    {
        log("url not support ");
...
void CHttpQuery::_QueryValidCode(const string& strAppKey, Json::Value &post_json_obj, CHttpConn *pHttpConn)
{
	string phone = post_json_obj["phone"].asString();

	//连接cache,生成验证码
	//CHttpClient httpClient;
	//httpClient.Post();

	//http://sdk999ws.eucp.b2m.cn:8080/sdkproxy/sendsms.action?cdkey=9SDK-EMY-0999-AAAAA&password=123456&phone=1333333333,13444444444&message=单发即时短信测试&addserial=10086

}

void CHttpQuery::_AssertValidCode(const string& strAppKey, Json::Value &post_json_obj, CHttpConn *pHttpConn)
{
	string phone = post_json_obj["phone"].asString();
...
void CHttpQuery::_QueryRegister(const string& strAppKey, Json::Value &post_json_obj, CHttpConn *pHttpConn)
{
	//还要再验证下验证码
    ...
    try
    {
        string user_name = post_json_obj["user_name"].asString();
        string password = post_json_obj["password"].asString();
        log("user_name: %s, password: %s",user_name.c_str(), password.c_str());

        CDbAttachData attach_data(ATTACH_TYPE_HANDLE, pHttpConn->GetConnHandle());
        IM::Register::IMRegisterReq msg;
        msg.set_user_name(user_name);
        ...
        pdu.SetCommandId(IM::BaseDefine::CID_OTHER_REGISTER_REQ);
        pConn->SendPdu(&pdu);
...
string CHttpQuery::UrlDecode(const string &sIn)
{
	string sOut;
