...
void CDBServConn::_HandleRegisterRsp(CImPdu *pPdu)
{
    ...
    log("result=%u", result_code);

    char* response_buf = NULL;
    if (result_code != 0)
    {
        response_buf = PackSendRegisterResult(HTTP_ERROR_CREATE_GROUP, HTTP_ERROR_MSG[10].c_str());
    }
    else
    {
        response_buf = PackSendRegisterResult(HTTP_ERROR_SUCCESS, HTTP_ERROR_MSG[0].c_str());
    }
    pHttpConn->Send(response_buf, (uint32_t)strlen(response_buf));
