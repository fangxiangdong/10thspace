...
static ConnMap_t g_test_server_conn_map;
...
{
	m_handle = netlib_connect(strIp.c_str(), nPort, imconn_callback, (void*)&g_test_server_conn_map);

        if (m_handle != NETLIB_INVALID_HANDLE) {
		g_test_server_conn_map.insert(make_pair(m_handle, this));
	}
    return  m_handle;
}
...
uint32_t ClientConn::register1(const string &strName, const string &strPass)
{
    /*CImPdu cPdu;
    ...
    cPdu.SetPBMsg(&msg);
    cPdu.SetServiceId(IM::BaseDefine::SID_REGISTER);
    cPdu.SetCommandId(IM::BaseDefine::CID_REGISTER_REQ_MSGSERVER);
    uint32_t nSeqNo = m_pSeqAlloctor->getSeq(ALLOCTOR_PACKET);
    cPdu.SetSeqNum(nSeqNo);
    SendPdu(&cPdu);
    return nSeqNo;*/
...
uint32_t ClientConn::sendBlog(uint32_t nFromId, const string& strMsgData)
{
    /*CImPdu cPdu;
    IM::Message::IMMsgBlog msg;
    msg.set_from_user_id(nFromId);
    msg.set_msg_id(0);
    msg.set_create_time(time(NULL));
    //msg.set_msg_type(nType);
    ...
    cPdu.SetPBMsg(&msg);
    cPdu.SetServiceId(IM::BaseDefine::SID_MSG);
    cPdu.SetCommandId(IM::BaseDefine::CID_MSG_BLOG);
    uint32_t nSeqNo = m_pSeqAlloctor->getSeq(ALLOCTOR_PACKET);
    cPdu.SetSeqNum(nSeqNo);
    SendPdu(&cPdu);
    return nSeqNo;*/
...
uint32_t ClientConn::getBlog(uint32_t nFromId)
{
    /*CImPdu cPdu;
    IM::Message::IMGetBlogListReq msg;
    msg.set_user_id(nFromId);
    msg.set_update_time(0);
    cPdu.SetPBMsg(&msg);
    cPdu.SetServiceId(IM::BaseDefine::SID_MSG);
    cPdu.SetCommandId(IM::BaseDefine::CID_MSG_BLOG_LIST_REQUEST);
    uint32_t nSeqNo = m_pSeqAlloctor->getSeq(ALLOCTOR_PACKET);
    cPdu.SetSeqNum(nSeqNo);
    SendPdu(&cPdu);
    return nSeqNo;*/
...
uint32_t ClientConn::findFriend(uint32_t nFromId,string &sName)
{
    CImPdu cPdu;
    IM::Buddy::IMSearchUserReq msg;
    msg.set_user_id(nFromId);
    msg.set_search_user_name(sName);
    cPdu.SetPBMsg(&msg);
    cPdu.SetServiceId(IM::BaseDefine::SID_BUDDY_LIST);
    cPdu.SetCommandId(IM::BaseDefine::CID_BUDDY_LIST_SEARCH_USER_REQUEST);
    uint32_t nSeqNo = m_pSeqAlloctor->getSeq(ALLOCTOR_PACKET);
    cPdu.SetSeqNum(nSeqNo);
    SendPdu(&cPdu);
    return nSeqNo;
...
uint32_t ClientConn::addFriend(uint32_t nFromId,uint32_t friend_id)
{
    CImPdu cPdu;
    ...
    cPdu.SetPBMsg(&msg);
    cPdu.SetServiceId(IM::BaseDefine::SID_BUDDY_LIST);
    cPdu.SetCommandId(IM::BaseDefine::CID_BUDDY_LIST_ADD_FRIEND_REQUEST);
    uint32_t nSeqNo = m_pSeqAlloctor->getSeq(ALLOCTOR_PACKET);
    cPdu.SetSeqNum(nSeqNo);
    SendPdu(&cPdu);
    return nSeqNo;
...
uint32_t ClientConn::followUser(uint32_t nFromId,uint32_t friend_id)
{
    CImPdu cPdu;
    ...
    cPdu.SetPBMsg(&msg);
    cPdu.SetServiceId(IM::BaseDefine::SID_BUDDY_LIST);
    cPdu.SetCommandId(IM::BaseDefine::CID_BUDDY_LIST_FOLLOW_USER_REQUEST);
    uint32_t nSeqNo = m_pSeqAlloctor->getSeq(ALLOCTOR_PACKET);
    cPdu.SetSeqNum(nSeqNo);
    SendPdu(&cPdu);
    return nSeqNo;
...
uint32_t ClientConn::online_cnt(uint32_t nUserId)
{
    /*CImPdu cPdu;
    IM::Message::IMGetOnlineUserCntReq msg;
    cPdu.SetPBMsg(&msg);
    cPdu.SetServiceId(IM::BaseDefine::SID_MSG);
    cPdu.SetCommandId(IM::BaseDefine::CID_MSG_GET_ONLINE_USER_CNT_RESPONSE);
    uint32_t nSeqNo = m_pSeqAlloctor->getSeq(ALLOCTOR_PACKET);
    cPdu.SetSeqNum(nSeqNo);
    SendPdu(&cPdu);
    return nSeqNo;*/
...
void ClientConn::Close()
{
	if (m_handle != NETLIB_INVALID_HANDLE) {
		netlib_close(m_handle);
                g_test_server_conn_map.erase(m_handle);
	}
	ReleaseRef();
}
...
//		printf("Heartbeat\n");
        ...
        case IM::BaseDefine::CID_BUDDY_LIST_ALL_USER_RESPONSE:
            _HandleUser(pPdu);
        break;
        case IM::BaseDefine::CID_BUDDY_LIST_SEARCH_USER_RESPONSE:
             _HandleUser2(pPdu);
             break;
        case IM::BaseDefine::CID_BUDDY_LIST_ADD_FRIEND_RESPONSE:
             _HandleAddFriend(pPdu);
             break;
        case IM::BaseDefine::CID_BUDDY_LIST_FOLLOW_USER_RESPONSE:
             //_HandleFollowUser(pPdu);
             break;
        /*case IM::BaseDefine::CID_MSG_BLOG_LIST_RESPONSE:
             _HandleBlogList(pPdu);
             break;*/
        ...
        case IM::BaseDefine::CID_MSG_DATA_ACK:
            _HandleSendMsg(pPdu);
            break;
        /*case IM::BaseDefine::CID_MSG_BLOG_ACK:
            _HandleSendBlog(pPdu);
            break;*/
        ...
        case IM::BaseDefine::CID_MSG_DATA:
            _HandleMsgData(pPdu);
            break;
        /*case IM::BaseDefine::CID_MSG_GET_ONLINE_USER_CNT_RESPONSE:
			_HandleOnlineCnt(pPdu);
			break;*/
        default:
...
void ClientConn::_HandleUser2(CImPdu* pPdu)
{
    IM::Buddy::IMSearchUserRsp msgResp;
    uint32_t nSeqNo = pPdu->GetSeqNum();
    if(msgResp.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength()))
    {
        uint32_t userCnt = msgResp.search_user_list_size();
        printf("get %d users\n", userCnt);
        list<IM::BaseDefine::UserInfo> lsUsers;
        for(uint32_t i=0; i<userCnt; ++i)
        {
            IM::BaseDefine::UserInfo cUserInfo = msgResp.search_user_list(i);
            lsUsers.push_back(cUserInfo);
        }
        m_pCallback->onGetChangedUser(nSeqNo, lsUsers);
    }
...
void ClientConn::_HandleAddFriend(CImPdu* pPdu)
{
    IM::Buddy::IMAddFriendRsp msgResp;
    uint32_t nSeqNo = pPdu->GetSeqNum();
    if(msgResp.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength()))
    {
        int32_t iRet = msgResp.result_code();
        printf("add friend result %d",iRet);
...
void ClientConn::_HandleBlogList(CImPdu* pPdu)
{
    /*IM::Message::IMGetBlogListRsp msgResp;
    uint32_t nSeqNo = pPdu->GetSeqNum();
    if(msgResp.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength()))
    {
		...
		printf("user id %u get blog %d:\n",nUserId, nMsgCnt);
		for(uint32_t i=0; i<nMsgCnt; ++i)
		{
			IM::BaseDefine::MsgInfo msgInfo = msgResp.msg_list(i);
			//lsMsg.push_back(msgInfo);
			//string strMsg(msgInfo.msg_data().c_str(), msgInfo.msg_data().length());
			printf("%d - %s\n",i,msgInfo.msg_data().c_str());
		}
    }
    else
    {
...
void ClientConn::_HandleSendBlog(CImPdu* pPdu)
{
    /*IM::Message::IMMsgBlogAck msgResp;
    uint32_t nSeqNo = pPdu->GetSeqNum();
    if(msgResp.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength()))
    {
        uint32_t nSendId = msgResp.user_id();
        ...
        //uint32_t nMsgId = msgResp.msg_id();
        //IM::BaseDefine::SessionType nType = msgResp.session_type();
        //m_pCallback->onSendBlog(nSeqNo, nSendId, nRecvId, nType, nMsgId);
        printf("send blog result");
    }
    else
    {
        m_pCallback->onError(nSeqNo, pPdu->GetCommandId(), "parse pb error");
...
void ClientConn::_HandleOnlineCnt(CImPdu* pPdu)
{
    IM::Message::IMGetOnlineUserCntRsp msgResp;
    uint32_t nSeqNo = pPdu->GetSeqNum();
    if(msgResp.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength()))
    {
        uint32_t nUserCnt = msgResp.online_user_cnt();
        //uint32_t nRecvId = msgResp.session_id();
        //uint32_t nMsgId = msgResp.msg_id();
        //IM::BaseDefine::SessionType nType = msgResp.session_type();
        //m_pCallback->onSendBlog(nSeqNo, nSendId, nRecvId, nType, nMsgId);
        printf("online users count %d\n", nUserCnt);
    }
    else
    {
        m_pCallback->onError(nSeqNo, pPdu->GetCommandId(), "parse pb error");
