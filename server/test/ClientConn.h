...
public:
    ...
    virtual void Close();
public:
    uint32_t register1(const string& strName, const string& strPass);
    ...
    uint32_t findFriend(uint32_t nFromId,string &sName);
    uint32_t addFriend(uint32_t nFromId,uint32_t friend_id);
    uint32_t sendBlog(uint32_t nFromId, const string& strMsgData);
    uint32_t followUser(uint32_t nFromId,uint32_t friend_id);
    uint32_t getBlog(uint32_t nFromId);

    uint32_t online_cnt(uint32_t nUserId);
...
private:
    ...
    void _HandleOnlineCnt(CImPdu* pPdu);
    
    void _HandleUser2(CImPdu* pPdu);
    void _HandleAddFriend(CImPdu* pPdu);
    void _HandleSendBlog(CImPdu* pPdu);
    void _HandleBlogList(CImPdu* pPdu);
    void _HandleFollowUser(CImPdu* pPdu);
