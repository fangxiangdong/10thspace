...
uint32_t CClient::login(const string& strName, const string& strPass)
{
    ...
    //return g_pConn->register1(strName, strPass);
}
...
void CClient::onGetChangedUser(uint32_t nSeqNo, const list<IM::BaseDefine::UserInfo> &lsUser)
{
    for(auto it=lsUser.begin(); it!=lsUser.end(); ++it)
    {
        ...
        *pUserInfo = *it;
        ...
        printf("%d | %s\n",pUserInfo->user_id(), pUserInfo->user_nick_name().c_str());
        if(it->status() != 3)
        {
...
uint32_t CClient::sendBlog(const string &strMsg)
{
    uint32_t nFromId = m_cSelfInfo.user_id();
    return g_pConn->sendBlog(nFromId, strMsg);
}

uint32_t CClient::getBlog()
{
    uint32_t nFromId = m_cSelfInfo.user_id();
    return g_pConn->getBlog(nFromId);
}

uint32_t CClient::findFriend(string &sName)
{
    uint32_t nFromId = m_cSelfInfo.user_id();
    return g_pConn->findFriend(nFromId, sName);
}

uint32_t CClient::addFriend(uint32_t friend_id)
{
    uint32_t nFromId = m_cSelfInfo.user_id();
    return g_pConn->addFriend(nFromId, friend_id);
}

uint32_t CClient::followUser(uint32_t friend_id)
{
    uint32_t nFromId = m_cSelfInfo.user_id();
    return g_pConn->followUser(nFromId, friend_id);
}

uint32_t CClient::online_cnt()
{
    uint32_t user_id = m_cSelfInfo.user_id();
    return g_pConn->online_cnt(user_id);
}
