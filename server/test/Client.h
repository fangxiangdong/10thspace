...
{
public:
    CClient(const string& strName, const string& strPass, const string strDomain="127.0.0.1");
    ~CClient();
public:
...
public:
    ...
    uint32_t findFriend(string &sName);
    uint32_t addFriend(uint32_t friend_id);
    uint32_t sendBlog(const string &strMsg);
    uint32_t getBlog();
    uint32_t followUser(uint32_t friend_id);
    uint32_t online_cnt();
