...
public class NaviTabButton extends FrameLayout {
    ...
    public NaviTabButton(Context context, AttributeSet attrs, int defStyle) {
        ...
        inflate = inflater.inflate(R.layout.tt_navi_tab_button, this, true);
    ...
    public int getLocalUnreadCnt(){
        String text = mNotify.getText().toString();
    ...
    public void setUnreadNotify(int unreadNum) {
        logger.i("unread#setUreadNotify -> unreadNum:%d", unreadNum);
        if (0 == unreadNum) {
            mNotify.setVisibility(View.INVISIBLE);
            TextView cnt = (TextView) inflate.findViewById(R.id.tab_unread_notify);
            cnt.setText("");
            return;
        }
