...
//                    }
    ...
    public void onRecvMessage(IMMessage.IMMsgData imMsgData) {
        ...
        if (imMsgData.getMsgType().equals(IMBaseDefine.MsgType.MSG_TYPE_GROUP_AUDIO)) {
            try {
                int fromUserId = imMsgData.getFromUserId();
        ...
        } else if (imMsgData.getMsgType().equals(IMBaseDefine.MsgType.MSG_TYPE_GROUP_AUDIO)) {

            int fromUserId = imMsgData.getFromUserId();//是否同意的用户
//            int userId = IMLoginManager.instance().getLoginId();//本机用户
            String msg = imMsgData.getMsgData().toStringUtf8();
            try {
                JSONObject jsonObject = new JSONObject(msg);
                jsonObject.put("fromUserId", fromUserId);
                notifyEvent.event = PriorityEvent.Event.MSG_AGREE_OR_DISGREE_ADD_FRIEND_RSP;
                notifyEvent.object = jsonObject;
            } catch (JSONException e) {
                e.printStackTrace();
            }
