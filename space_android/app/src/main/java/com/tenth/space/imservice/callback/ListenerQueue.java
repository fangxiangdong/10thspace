package com.tenth.space.imservice.callback;
...
public class ListenerQueue {
    ...
    public void onStart(){
        logger.i("ListenerQueue#onStart run");
        stopFlag = false;
        startTimer();
    }
    public void onDestory(){
        logger.i("ListenerQueue#onDestory ");
        callBackQueue.clear();
        stopTimer();
    }
        ...
        for (java.util.Map.Entry<Integer, Packetlistener> entry : callBackQueue.entrySet()) {
            ...
            try {
                if (timeRange >= packetlistener.getTimeOut()) {
                    logger.i("ListenerQueue#find timeout msg");
                ...
                logger.i("ListenerQueue#timerImpl onTimeout is Error,exception is %s", e.getCause());
            }
        }
    }
    ...
    public void push(int seqNo,Packetlistener packetlistener){
        if(seqNo <=0 || null==packetlistener){
            logger.i("ListenerQueue#push error, cause by Illegal params");
            return;
        }
