        ...
        public void onIMServiceConnected() {
            ...
            try {
                do {
                    if (imService == null) {
                        //后台服务启动链接失败
                        LogUtils.d("LoginActivity------启动服务失败");
...
//        }
    ...
    private void onLoginSuccess() {
        ...
        //加载网络用户默认图片，有就上传到阿里云，没有就用默认图片替代
        Utils.checkAndUpLoadPicture(this);
        loginSuccess = true;
    ...
    private void getUnreadFriendRequstCnt() {
        IMBuddy.IMAddFriendUnreadCntReq msg = IMBuddy.IMAddFriendUnreadCntReq.newBuilder()
                .setUserId(IMLoginManager.instance().getLoginId())
                .build();
        int sid = IMBaseDefine.ServiceID.SID_BUDDY_LIST_VALUE;
        int cid = IMBaseDefine.BuddyListCmdID.CID_BUDDY_LIST_ADD_FRIEND_UNREAD_CNT_REQUEST_VALUE;
