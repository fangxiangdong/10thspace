...
#import "MTTPathCover.h"
#import "MTTWaterDropRefresh.h"

NSString *const MTTUserNameKey = @"MTTUserName";
NSString *const MTTBirthdayKey = @"MTTBirthday";
...
@property (nonatomic, strong) MTTWaterDropRefresh *waterDropRefresh;

@property (nonatomic, assign) CGFloat showUserInfoViewOffsetHeight;

@end

@implementation MTTPathCover
...
- (void)setInfo:(NSDictionary *)info {
    NSString *userName = [info valueForKey:MTTUserNameKey];
    if (userName) {
        self.userNameLabel.font = [UIFont preferredFontForTextStyle:UIFontTextStyleTitle1];
        self.userNameLabel.adjustsFontForContentSizeCategory = YES;
        self.userNameLabel.text = userName;
    }
    NSString *birthday = [info valueForKey:MTTBirthdayKey];
    if (birthday) {
        self.birthdayLabel.font = [UIFont preferredFontForTextStyle:UIFontTextStyleTitle2];
        self.birthdayLabel.adjustsFontForContentSizeCategory = YES;
...
- (void)_setup {
    ...
    _waterDropRefresh = [[MTTWaterDropRefresh alloc] initWithFrame:CGRectMake(33, CGRectGetHeight(self.bounds) - waterDropRefreshHeight, waterDropRefreshWidth, waterDropRefreshHeight)];
...
- (void)initWaterView {
    __weak MTTPathCover *wself =self;
