    ...
    self.bar.Block = ^(int buttonIndex){
        if (buttonIndex == 0) {
            ...
            for (int i =0; i<[self.choosePhotosArray count]; i++) {
                ALAsset *result = [self.choosePhotosArray objectAtIndex:i];
            ...
            [self.button setTitle:[NSString stringWithFormat:@"发送(%ld)",(unsigned long)[self.photos count]] forState:UIControlStateNormal];
            [self.button setTitle:[NSString stringWithFormat:@"发送(%ld)",(unsigned long)[self.photos count]] forState:UIControlStateSelected];
            ...
            if ([self.choosePhotosArray count] >0) {
                ...
                [HUD showAnimated:YES whileExecutingBlock:^{
                    for (int i = 0; i<[self.choosePhotosArray count]; i++) {
                        ...
                        ALAsset *asset = [self.choosePhotosArray objectAtIndex:i];
...
- (void) gridView: (AQGridView *) gridView didSelectItemAtIndex: (NSUInteger) index
{
    ...
    if ([self.choosePhotosArray containsObject:asset]) {
        ...
        if ([self.choosePhotosArray count] == 9) {
            return;
        }
        [cell setCellIsToHighlight:YES];
