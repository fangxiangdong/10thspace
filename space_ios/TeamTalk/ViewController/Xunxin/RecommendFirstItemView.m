...
@interface RecommendFirstItemView()<AVCaptureVideoDataOutputSampleBufferDelegate>
...
@implementation RecommendFirstItemView
...
- (void)listenForEvents
{
    ...
    /** 当前窗口显示的是自己的view*/
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(currentViewIsRecommendView:) name:@"RecommendView" object:nil];
    /** 当前窗口显示的不是自己的view*/
...
- (void)currentViewIsRecommendView:(NSNotification *)note
{
    NSString *cameraStatus = [[NSUserDefaults standardUserDefaults] objectForKey:@"currentCameraStatus"];
    // 相机打开状态
    if ([cameraStatus isEqualToString:@"cameraOpen"]) {
