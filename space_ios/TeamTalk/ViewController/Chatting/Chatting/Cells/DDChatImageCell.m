...
{
    // 获取气泡设置
    [super setContent:content];//NSLog(@"----msgContent == %@",content.msgContent);
    //设置气泡位置
    ...
    if(content.msgContentType == DDMessageTypeImage)
    {
       ...
       NSDictionary* messageContent2 = [NSDictionary initWithJsonString:[SecurityUtil decryptAESData:content.msgContent]];
        if (!messageContent)
        {
            NSString* urlString = content.msgContent;
            urlString =[SecurityUtil decryptAESData:urlString];
            ...
            NSURL* url = [NSURL URLWithString:[NSString stringWithFormat:@"%@?x-oss-process=image/resize,m_fixed,h_150,w_100",urlString]];
        ...
        else if (messageContent2[DD_IMAGE_LOCAL_KEY])
        {
            //加载本地图片
            NSString* localPath = messageContent2[DD_IMAGE_LOCAL_KEY];
            NSData* data = [[MTTPhotosCache sharedPhotoCache] photoFromDiskCacheForKey:localPath];
            UIImage *image = [[UIImage alloc] initWithData:data];
            [self.msgImgView setImage:image];
            ...
            [self showSending];
            [self.msgImgView sd_setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@?x-oss-process=image/resize,m_fixed,h_150,w_100",url]] completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, NSURL *imageURL) {
...
- (CGSize)sizeForContent:(MTTMessageEntity*)content
{
    float height = 150;
    float width = 100;
    NSString* urlString =[SecurityUtil decryptAESData:content.msgContent]  ;
    urlString = [urlString stringByReplacingOccurrencesOfString:DD_MESSAGE_IMAGE_PREFIX withString:@""];
    urlString = [urlString stringByReplacingOccurrencesOfString:DD_MESSAGE_IMAGE_SUFFIX withString:@""];
