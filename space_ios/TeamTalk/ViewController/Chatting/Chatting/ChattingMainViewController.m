...
-(void)sendImageMessage:(MTTPhotoEnity *)photo Image:(UIImage *)image
{
    ...
    NSString *postImage =  [messageContentDic[DD_IMAGE_LOCAL_KEY] stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
    
    //NSLog(@"--postImage == %@",postImage);
...
//    - (void)uploadMsgImageToAliYunOSSWithContent:(NSString *)imageKey andUserID:(NSString*)userID success:(void(^)(NSString *fileURL))success failure:(void(^)(NSError *error))failure
   [[DDSendPhotoMessageAPI sharedPhotoCache]uploadMsgImageToAliYunOSSWithContent:postImage andUserID:TheRuntime.user.objID success:^(NSString *imageURL) {
...
- (void)textViewEnterSend
{
  ...
  text= [SecurityUtil encryptAESData:text];
    NSString* parten = @"\\s";
...
-(void)insertEmojiFace:(NSString *)string
{
    ...
    MTTMessageEntity *message = [MTTMessageEntity makeMessage: [SecurityUtil encryptAESData:string] Module:self.module MsgType:msgContentType];
    [self.tableView reloadData];
    //[self.chatInputView.textView setText:nil];
    [[MTTDatabaseUtil instance] insertMessages:@[message] success:^{
        DDLog(@"消息插入DB成功");
    ...
    [self sendMessage: [SecurityUtil encryptAESData:string] messageEntity:message];
...
- (UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([object isKindOfClass:[MTTMessageEntity class]])
    {
        ...
        message.msgContentType=DDMEssageEmotion;
...
- (UITableViewCell*)p_imageCell_tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath message:(MTTMessageEntity*)message
{
    ...
    [cell setTapInBubble:^{
        NSString *originUrl =[SecurityUtil decryptAESData:message.msgContent];
        ...
        [self.module.showingMessages enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
            if ([obj isKindOfClass:[MTTMessageEntity class]])
            {
                ...
                if(message.msgContentType == DDMessageTypeImage){
                    NSString* urlString = [SecurityUtil decryptAESData:message.msgContent] ;
                    ...
                    }else if ([message.msgContent rangeOfString:@"\"local\" : "].length >0)
                    {
                        NSData* data = [message.msgContent dataUsingEncoding:NSUTF8StringEncoding];
                        NSDictionary* dic = [NSJSONSerialization JSONObjectWithData:data options:0 error:nil];
        ...
        [photos enumerateObjectsUsingBlock:^(NSURL *obj, NSUInteger idx, BOOL *stop) {
            //NSLog(@"---url == %@--%@",obj,[MWPhoto photoWithURL:obj]);
