...
- (void)setLastMessage:(NSString*)message
{
    ...
    else
    {
        ...
        if([message hasPrefix:@"["]  )
        {
            ...
            [_lastmessageLabel setText:[SecurityUtil decryptAESData:message]];
            }
...
-(void)setShowSession:(MTTSessionEntity *)session
{
    ...
    if(session.sessionType == SessionTypeSessionTypeSingle || session.sessionType == SessionTypeSessionTypeGroup){
        [self setUnreadMessageCount:session.unReadMsgCount];
    }
    else{
