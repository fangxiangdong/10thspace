...
{
    return [NSString stringWithFormat:@"%@%ld",USER_PRE,(unsigned long)userID];
}
...
-(id)initWithPB:(UserInfo *)pbUser
{
    ...
    if (self) {
        ...
        self.userID = [[self class] pbUserIdToLocalID:pbUser.userId];
        ...
        self.department = [NSString stringWithFormat:@"%u",(unsigned int)pbUser.departmentId];
        self.departId   = @"";
        ...
        self.userID     = [NSString stringWithFormat:@"%u",(unsigned int)pbUser.userId];
        self.relation=[NSString stringWithFormat:@"%d",(int)pbUser. relation];
        self.fansCount=pbUser.fansCnt;
    }
    return self;
}
