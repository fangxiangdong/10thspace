...
+(MTTMessageEntity *)makeMessageFromPBData:(IMMsgData *)data
{
    ...
    if(msg.msgType == MsgTypeMsgTypeGroupAudio || msg.msgType == MsgTypeMsgTypeGroupText){
        type = SessionTypeSessionTypeGroup;
    }
    else if(msg.msgType == MsgTypeMsgTypeSingleAudio || msg.msgType == MsgTypeMsgTypeSingleText ){
        type = SessionTypeSessionTypeSingle;
    }
    else{
        type = SessionTypeSessionTypeSystem;
        
        //@[object.senderId,@(object.msgID),object.sessionId,@(object.sessionType)
    ...
    }else
    {
        ...
        if(msg.sessionType == SessionTypeSessionTypeSingle || msg.sessionType == SessionTypeSessionTypeGroup){
...
//                Free(pOut);
    ...
    NSString *destr=[SecurityUtil decryptAESData:msg.msgContent];
     if ([destr hasPrefix:@"&$#@~^@[{:"])
     {
