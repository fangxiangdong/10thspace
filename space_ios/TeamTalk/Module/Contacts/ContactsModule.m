...
+(id)shark
{
    static ContactsModule* module;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        module = [[ContactsModule alloc] init];
...
- (instancetype)init
{
    ...
    if (self) {
        ...
        [[NSNotificationCenter defaultCenter] addObserver:self
                                                 selector:@selector(n_receiveAddFriendMessageNotification:)
                                                     name:DDNotificationReceiveAddFriendMessage
                                                   object:nil];
    ...
    for (MTTUserEntity * user in [[DDUserModule shareInstance] getAllMaintanceUser]) {
        ...
        if(![user.objID isEqualToString:TheRuntime.user.objID]){
            ...
            if(user.pyname.length == 0){
                fl = [user.name substringWithRange:NSMakeRange(0, 1)];
            }
            else{
                fl = [user.pyname substringWithRange:NSMakeRange(0, 1)];
            }
            */
...
-(NSMutableDictionary *)sortByAttentionPy
{
    NSMutableDictionary *dic = [NSMutableDictionary new];
    
    for (MTTUserEntity * user in [[DDUserModule shareInstance] getAllAttention]) {
        //NSLog(@"--- user == %@--%@--%@",user.userID,user.name,user.pyname);
        // 排除用户名为空和本身
        if(![user.objID isEqualToString:TheRuntime.user.objID]){
...
-(void)n_receiveAddFriendMessageNotification:(NSNotification*)notification
{
    IMAddFriendData*data=[notification object];
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector( ContactsModuleUpdate:)]) {
        [self.delegate ContactsModuleUpdate:model];
    }
}
