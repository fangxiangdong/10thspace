...
//  Copyright (c) 2014年 landu. All rights reserved.
//
...
+ (NSString *)doCipher:(NSString *)sTextIn key:(NSString *)sKey
    ...
    if (encryptOrDecrypt == kCCDecrypt) {
        ...
        dTextIn = [[GTMBase64 decodeData:[sTextIn dataUsingEncoding:NSUTF8StringEncoding]] mutableCopy];
    }
    else{
    ...
    const void *vkey = (const void *) [sKey UTF8String];
   
    bufferPtrSize1 = (plainTextBufferSize + kCCBlockSizeDES) & ~(kCCBlockSizeDES -1);
    bufferPtr1 = malloc(bufferPtrSize1 * sizeof(uint8_t));
    memset((void *)bufferPtr1, 0x0, bufferPtrSize1);
...
//       NSLog(@"99:%d",[dTextIn bytes]);
//        NSLog(@"%d,%d",bufferPtr1,movedBytes1);
        NSData *dResult = [NSData dataWithBytes:(const void *)bufferPtr1 length:(NSUInteger)movedBytes1];
       
        sResult = [GTMBase64 stringByEncodingData:dResult];
    }
    return sResult;
}
