...
// 新添加
+(void) myJsonFormPOSTRequest:(NSString *)url param:(NSDictionary *)param success:(void (^)(id successObject))success failure:(void (^)(NSError *error))failure;

+(void) myJsonFormRequest:(NSString *)url param:(NSDictionary *)param fromBlock:(void (^)(id <AFMultipartFormData> formData))block success:(void (^)(id successObject))success failure:(void (^)(NSError *))failure;
