...
typedef void(^DeleteFriendCompletion)(BOOL success);
typedef void(^DeleteBlogsCompletion)(BOOL success);
...
typedef void (^LoadAllBlogsComplection)(NSArray *blogs,NSError *error);
typedef void (^UpdataBlogsComplection)(NSError *error);
typedef void (^InsertBLogsComplection)(NSError *error);

//------ 自己加的---------
@interface MTTDatabaseUtil(Blogs)
...
-(void)updataBlogs:(NSArray*)blogs completion:(UpdataBlogsComplection)complection;
