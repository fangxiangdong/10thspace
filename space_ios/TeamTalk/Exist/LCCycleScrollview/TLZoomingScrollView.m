...
- (void)showAnimation
{
    if (self.imgSize.width == 0 || self.imgSize.height == 0) return;
    
    CGFloat width = [UIScreen mainScreen].bounds.size.width;
    CGFloat height = width * self.imgSize.height / self.imgSize.width;
...
// 设置预览大图
- (void)setBigPreviewImage:(NSString *)bigImageURL
{
    // 第一次需要动画, 左右滑动不需要动画
    if (self.isAnimation) {
    ...
    [_imageView sd_setImageWithURL:[NSURL URLWithString:bigImageURL] placeholderImage:_imageView.image options:SDWebImageRetryFailed];
}
